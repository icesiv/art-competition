...
<body class="from-primary to-secondary min-h-screen">
    ...
    <nav class="bg-white/95 backdrop-blur-md shadow-lg sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <!-- Logo/Brand -->
                <a href="{{ route('registration.index') }}" class="flex items-center space-x-2 text-primary hover:text-secondary transition-colors">
                ...
                <div class="hidden md:flex items-center space-x-4">
                    <button onclick="openDownloadModal()" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:shadow-lg transform hover:-translate-y-0.5 transition-all font-semibold">
                    প্রবেশপত্র ডাউনলোড
                    </button>
                </div>
        ...
        <div id="mobileMenu" class="hidden md:hidden bg-white border-t">
            <div class="px-4 py-3 space-y-2">
                <button onclick="openDownloadModal()" class="w-full text-left bg-gradient-to-r from-green-500 to-teal-500 text-white px-4 py-2 rounded-lg hover:shadow-lg transition-all font-semibold">
                    📥 প্রবেশপত্র ডাউনলোড
                </button>
            </div>
        </div>
    ...
    <!-- Download Modal -->
    <div id="downloadModal" class="hidden fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 animate-fadeIn">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto transform transition-all animate-slideUp">
            <div class="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center rounded-t-2xl">
                ...
                <button onclick="closeDownloadModal()" class="text-gray-400 hover:text-gray-600 text-3xl leading-none transition-colors">
                    &times;
                </button>
            </div>
            ...
            <form method="POST" action="{{ route('registration.download') }}" class="p-6 space-y-4">
                ...
                <button type="submit" class="w-full bg-gradient-to-r from-green-500 to-teal-500 text-white px-6 py-3 rounded-lg hover:shadow-lg transform hover:-translate-y-0.5 transition-all font-semibold text-lg">
                    ডাউনলোড করুন
                </button>
            </form>
        </div>
