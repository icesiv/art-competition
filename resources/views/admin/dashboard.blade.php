...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-2xl p-6 md:p-8 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <!-- Total Registrations -->
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg transform hover:scale-105 hover:shadow-2xl transition-all duration-300 cursor-pointer">
            ...
            <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg transform hover:scale-105 hover:shadow-2xl transition-all duration-300 cursor-pointer">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        <p class="text-green-100 text-sm font-medium mb-2 uppercase tracking-wide">আজকের রেজিস্ট্রেশন</p>
                        <h3 class="text-5xl font-bold mb-1">{{ $registrations->where('created_at', '>=', today())->count() }}</h3>
            ...
            <!-- Latest Registration -->
            <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg transform hover:scale-105 hover:shadow-2xl transition-all duration-300 cursor-pointer">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
        ...
        <div class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-6 border-2 border-gray-200">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <a href="{{ route('admin.export.csv') }}" class="group bg-gradient-to-r from-teal-500 to-cyan-500 text-white px-6 py-4 rounded-xl hover:shadow-xl transform hover:-translate-y-1 transition-all font-semibold text-center flex items-center justify-center space-x-3">
                    ...
                    <span>CSV ফরম্যাট</span>
                </a>
                <a href="{{ route('admin.export.excel') }}" class="group bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-6 py-4 rounded-xl hover:shadow-xl transform hover:-translate-y-1 transition-all font-semibold text-center flex items-center justify-center space-x-3">
                    <svg class="w-6 h-6 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gradient-to-r from-primary/10 to-secondary/10">
                    <tr>
                        <th class="px-4 md:px-6 py-4 text-left text-xs md:text-sm font-bold text-primary uppercase tracking-wider whitespace-nowrap">রেজিস্ট্রেশন আইডি</th>
                        <th class="px-4 md:px-6 py-4 text-left text-xs md:text-sm font-bold text-primary uppercase tracking-wider whitespace-nowrap">নাম</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-primary/5 transition-colors">
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs md:text-sm font-semibold bg-primary/10 text-primary border border-primary/20">
                                {{ $reg->registration_id }}
                            </span>
                        </td>
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                            <div class="font-medium text-gray-900 text-sm md:text-base">{{ $reg->name }}</div>
                            <div class="text-xs text-gray-500 md:hidden">{{ $reg->grade }}</div>
                        </td>
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap text-gray-600 text-sm hidden md:table-cell">{{ $reg->grade }}</td>
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap text-gray-600 text-sm hidden lg:table-cell">{{ $reg->parents_name }}</td>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex space-x-2">
                    @if ($registrations->onFirstPage())
                        <span class="px-4 py-2 bg-gray-200 text-gray-500 rounded-lg cursor-not-allowed text-sm font-medium">পূর্ববর্তী</span>
                    @else
                        <a href="{{ $registrations->previousPageUrl() }}" class="px-4 py-2 bg-white border-2 border-primary text-primary rounded-lg hover:bg-primary hover:text-white transition-all font-semibold text-sm">পূর্ববর্তী</a>
                    @endif

                    @if ($registrations->hasMorePages())
                        <a href="{{ $registrations->nextPageUrl() }}" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-secondary transition-all font-semibold text-sm">পরবর্তী</a>
                    @else
                        <span class="px-4 py-2 bg-gray-200 text-gray-500 rounded-lg cursor-not-allowed text-sm font-medium">পরবর্তী</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-2xl p-12 md:p-16 text-center">
        ...
        <a href="{{ route('registration.index') }}" class="inline-flex items-center space-x-2 bg-gradient-to-r from-primary to-secondary text-white px-6 py-3 rounded-lg hover:shadow-lg transition-all font-semibold">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
